...
export function arcHelp(): Envelope {
  return {
    ...
    body: [
      ...
      "      [--disposition landed|failed|withdrawn] --pg",
      ...
      "        `--disposition` (ADR-0564 D1) records what the close MEANT, which is what the board",
      "        paints: green for `landed`, red for `failed`, and `withdrawn` for work that stopped",
      "        rather than lost (a duplicate, a superseded plan) — NOT a softer `failed` (D3). Omit it",
      "        and the reading derives from `--pr`; record `landed` when the landing was a DECISION,",
      "        an arc edit or knowledge artifacts, which is a real landing that carries no PR.",
      "",
