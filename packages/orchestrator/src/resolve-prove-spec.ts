...
export function realPrompts(
  ...
  // The set of scope entries this RUNTIME actually has write authority over, beyond the spotlight
  // itself. Codex's promotion authority is the finite manifest — a wildcard/glob-magic entry is a
  // hook-wall pattern only, never a Codex promotion target (`codexPromotionManifest` filters it the
  // same way) — so only its LITERAL siblings are ever named. Claude's actual write wall
  // (`PathWriteScope`) is glob-based and genuinely grants a wildcard match, so nothing is filtered
  // for it: naming only the literals would silently narrow what Claude is truthfully told it may
  // write (discrepancy #3). Either way the spotlight itself is excluded from "additional" — whether
  // or not it is itself one of the literal scope entries (discrepancy #1: a spotlight matched ONLY
  // by a wildcard is never itself in this list, so it must never be subtracted twice).
  const additionalScope = (spotlight: string, scopeGlobs: readonly string[]): string[] => {
    const candidates = codexRuntime ? scopeGlobs.filter((g) => !CODEX_GLOB_MAGIC.test(g)) : [...scopeGlobs];
    return candidates.filter((g) => g !== spotlight);
  };
  // Singular case (nothing beyond the spotlight) is byte-identical to the old literal for every
  // migrated single-file node, for both runtimes.
  const additionalSourceGlobs = additionalScope(real.sourceFile, real.scope.sourceGlobs);
  const sourcesNamed =
    additionalSourceGlobs.length === 0
      ? `\`${real.sourceFile}\``
      : `\`${real.sourceFile}\` and the other source files in your scope (matching ` +
        `${additionalSourceGlobs.map((g) => `\`${g}\``).join(", ")})`;
  ...
  const additionalTestGlobs = additionalScope(real.testFile, real.scope.testGlobs);
  const testsNamed =
    additionalTestGlobs.length === 0
      ? `\`${real.testFile}\``
      : `\`${real.testFile}\` (the required output) and the other test files in your permitted ` +
        `scope (matching ${additionalTestGlobs.map((g) => `\`${g}\``).join(", ")}) — IMPLEMENT may ` +
  ...
  return {
    ...
    implement:
      `${header}\n\n${conventions}${contractsImplement}${guidance}\n\nPhase IMPLEMENT — read ${testsNamed}, ` +
      // Discrepancy #2: an optional additional literal source file the manifest genuinely permits
      // must not be hidden behind "write ONLY <spotlight>" — name the full permitted set instead.
      `then ${
        additionalSourceGlobs.length === 0 ? `write ONLY \`${real.sourceFile}\`` : `write ${sourcesNamed}`
      } so that test passes. Writes to the test file are ` +
