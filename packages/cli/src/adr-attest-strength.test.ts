/**
 * The arms that discriminate `adr attest`'s remaining decisions — each written against a fixture
...
const depsFor = (store: InMemoryStore, writable = true): AdrAttestDeps => ({
  store,
  writable,
...
test("adr attest: only `adr` rows are read — a sibling kind never enters the denominator", async () => {
...
test("adr attest: the owner's words are TRIMMED before they are stored", async () => {
  const store = new InMemoryStore();
  await seed(store, 100);
  await adrAttest("100", { basis: "owner-directed", ownerSaid: "  do the thing\n" }, depsFor(store));
  assert.equal((await authorityOf(store, 100))?.["ownerSaid"], "do the thing");
});

test("adr attest: whitespace-only owner words are treated as NO quote, and the refusal says so", async () => {
  ...
  const store = new InMemoryStore();
  await seed(store, 100);
  const env = await adrAttest("100", { basis: "owner-directed", ownerSaid: "   \n  " }, depsFor(store));
  assert.equal(env.ok, false);
  assert.match(env.body, /must quote the owner verbatim/);
  assert.equal(await authorityOf(store, 100), undefined);
});

test("adr attest: a draft breaking SEVERAL schema rules reports each on its own line", async () => {
...
test("adr attest: the index counts TRANSCRIBED stamps, not merely non-quoted ones", async () => {
...
test("adr attest --backfill: the failure block is separated from the counts by a blank line", async () => {
  const store = new InMemoryStore();
  const id = idOf(100);
  await store.upsertDoc({
    id,
...
test("adr attest <n>: the follow-up offered depends on whether the record is already stamped", async () => {
  ...
  assert.deepEqual((await adrAttest("100", {}, depsFor(store))).next, [
    "storytree library artifact adr-0100",
    "storytree adr attest 100 --basis agent-derived --pg",
  ]);
  assert.deepEqual((await adrAttest("200", {}, depsFor(store))).next, [
