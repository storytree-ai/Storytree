/**
 * GOLDEN RENDERS for `storytree adr attest` — every shape's whole body and its whole `next:` list.
 *
 ...
 * is what makes the wording a thing the suite holds rather than a thing nobody is watching. Regenerate
 * by reading the new body, not by loosening the assertion.
 *
 * ⚠ A SHAPE NOT PINNED HERE KEEPS ITS WHOLE LITERAL SET ALIVE. When you add a branch to
 * `adr-attest.ts` that renders anything, add its case below in the same landing.
 */
...
import { adrAttest, type AdrAttestDeps } from "./adr-attest.js";
...
const deps = (store: InMemoryStore, writable = true): AdrAttestDeps => ({
  store,
  writable,
...
async function coverageAfterBackfill(): Promise<Awaited<ReturnType<typeof adrAttest>>> {
  const store = await corpus();
  await adrAttest(undefined, { backfill: true }, deps(store));
  return adrAttest(undefined, {}, deps(store));
}

async function readTranscribedFixture(): Promise<Awaited<ReturnType<typeof adrAttest>>> {
  const store = new InMemoryStore();
  await seed(store, 100, "accepted.", {
    authority: { basis: "owner-directed", scribedBy: "cli@x", at: "2026-09-05", transcribedFromProse: true },
  });
  return adrAttest("100", {}, deps(store));
}

test("golden render — emptyStore", async () => {
  const env = await adrAttest(undefined, {}, deps(new InMemoryStore()));
...
test("golden render — dryRun", async () => {
  const env = await adrAttest(undefined, { backfill: true }, deps(await corpus(), false));
  ...
  assert.deepEqual(env.next ?? [], ["pnpm db:up","storytree adr attest --backfill --pg","storytree adr attest"]);
});

test("golden render — backfillApplied", async () => {
  const env = await adrAttest(undefined, { backfill: true }, deps(await corpus()));
  assert.equal(env.ok, true);
  ...
  assert.deepEqual(env.next ?? [], ["storytree adr attest","storytree adr list --current"]);
});

test("golden render — backfillRestamp", async () => {
  const env = await adrAttest(undefined, { backfill: true, restamp: true }, deps(await corpus()));
  assert.equal(env.ok, false);
  assert.equal(env.body, "--restamp is refused on --backfill, and not merely unimplemented.\n\nA bulk pass that could overwrite would replace an AUTHORED stamp — a quoted owner directive,\ngiven in a session where he actually spoke — with a phrase match read off prose. That is the\ninversion of ADR-0519 D5, which exists to TRANSCRIBE what nobody captured, never to overwrite\nwhat somebody did. Re-stamp records one at a time, where the choice is visible.");
  assert.deepEqual(env.next ?? [], ["storytree adr attest --backfill"]);
});

test("golden render — backfillWithNumber", async () => {
  const env = await adrAttest("100", { backfill: true }, deps(await corpus()));
  assert.equal(env.ok, false);
...
test("golden render — readTranscribed", async () => {
  ...
  assert.deepEqual(env.next ?? [], ["storytree library artifact adr-0100","storytree adr attest"]);
});

test("golden render — readUnstampedClassifiable", async () => {
  const env = await adrAttest("100", {}, deps(await corpus()));
  assert.equal(env.ok, true);
  assert.equal(env.body, "ADR-0100 — unstamped — nobody has recorded whose call this was\n\nThe backfill would read it as `owner-directed` from its own prose (`storytree adr attest --backfill` to see the whole pass).");
  assert.deepEqual(env.next ?? [], ["storytree library artifact adr-0100","storytree adr attest 100 --basis agent-derived --pg"]);
});

test("golden render — readUnstampedOpaque", async () => {
  const env = await adrAttest("300", {}, deps(await corpus()));
  assert.equal(env.ok, true);
  ...
  assert.deepEqual(env.next ?? [], ["storytree library artifact adr-0300","storytree adr attest 300 --basis agent-derived --pg"]);
});

test("golden render — stamped", async () => {
  const env = await adrAttest("300", { basis: "owner-ratified", ownerSaid: "do the thing" }, deps(await corpus()));
  assert.equal(env.ok, true);
  assert.equal(env.body, "stamped adr-0300:\n  owner-ratified (quoted owner directive) · scribed by cli@claude/test on 2026-09-05\n\nThe owner's words, verbatim:\n  > do the thing");
  assert.deepEqual(env.next ?? [], ["storytree library artifact adr-0300","storytree adr attest"]);
});

test("golden render — restamped", async () => {
  const env = await adrAttest("500", { basis: "agent-flipped", restamp: true }, deps(await corpus()));
  assert.equal(env.ok, true);
  assert.equal(env.body, "RE-STAMPED adr-0500:\n  agent-flipped (declared, no quote) · scribed by cli@claude/test on 2026-09-05");
  assert.deepEqual(env.next ?? [], ["storytree library artifact adr-0500","storytree adr attest"]);
});

test("golden render — alreadyStamped", async () => {
  const env = await adrAttest("500", { basis: "agent-derived" }, deps(await corpus()));
  assert.equal(env.ok, false);
  ...
  assert.deepEqual(env.next ?? [], ["storytree adr attest 500","storytree library artifact adr-0500"]);
});

test("golden render — badBasis", async () => {
  const env = await adrAttest("300", { basis: "nope" }, deps(await corpus()));
  assert.equal(env.ok, false);
  assert.equal(env.body, "--basis must be one of owner-directed | owner-ratified | agent-derived | agent-flipped (got \"nope\").");
  assert.deepEqual(env.next ?? [], ["storytree adr attest 300"]);
});

test("golden render — notWritable", async () => {
  const env = await adrAttest("300", { basis: "agent-derived" }, deps(await corpus(), false));
  assert.equal(env.ok, false);
  assert.equal(env.body, "attesting writes to the shared store — run with --pg (and bring the DB up first: pnpm db:up).");
  assert.deepEqual(env.next ?? [], ["pnpm db:up","storytree adr attest 300 --basis agent-derived --pg"]);
});

test("golden render — badNumber", async () => {
  const env = await adrAttest("banana", {}, deps(await corpus()));
  assert.equal(env.ok, false);
  assert.equal(env.body, "expected a decision NUMBER (got \"banana\").");
  assert.deepEqual(env.next ?? [], ["storytree adr attest 519","storytree adr attest"]);
});

test("golden render — unknownRow", async () => {
  const env = await adrAttest("777", {}, deps(await corpus()));
  assert.equal(env.ok, false);
  assert.equal(env.body, "no decision row \"adr-0777\" in the store.");
  assert.deepEqual(env.next ?? [], ["storytree adr list --current","storytree adr attest"]);
});

test("golden render — ownerBasisNoQuote", async () => {
  const env = await adrAttest("300", { basis: "owner-directed" }, deps(await corpus()));
  assert.equal(env.ok, false);
