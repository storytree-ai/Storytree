...
test("node build without an id, and bare `node`, are help/guidance", async () => {
  ...
  // And library-focus-subgraph (stories/library-tech-tree-overlay/library-focus-subgraph.md —
  // ADR-0185 increment 3, ADR-0070 two-stage): the NET-NEW studio vitest `real:` arm over
  // LibraryFocusGraph.tsx + focusGraph.ts (references[] both-ways adjacency, dagre LR, kind-in-node
  // plaques, colour = state); the forest-cozy look + the two-pane peek mounting are the story's
  // operator-attested UAT leg.
  ...
  // And the two `terminal-repo-picker`-story capabilities (stories/terminal-repo-picker/*.md — ADR-0174
  // follow-on: the embedded terminal opens in a repo the user PICKS): repo-selection (NET-NEW node:test
  // over injected DirProbe + SelectionStore ports — the validate/persist/read/resolve-cwd lifecycle, no
  // real node:fs at test time) and repo-picker-panel (NET-NEW vitest jsdom over a mocked desktopRepo
  // bridge — the renderer picker, ADR-0070 geometry-machine-proven; its explicit vitest `proofCommand`
  // since the studio suite is vitest, not node:test). The real node:fs/userData adapters + the native
  // dialog + the `desktopRepo` preload bridge + the TreeView mount + threading resolveCwd into the
  // EXISTING terminal spawn are operator-attested glue (no `real:` arm); the native-dialog / opens-in-the-
  // picked-repo / survives-relaunch / look legs are the story's operator-attested UAT.
  assert.match(
    bare.body,
    /REAL-buildable nodes: +accept-reject-suggestion-api, act2-beat-director, ambient-integration, auto-grow-input, backend-chat-reset-route, block-position-comment-anchor, boot-read-routes, boundhash-on-verdict, brokered-local-uat-signing, builder-role, builder-spawn-dispatch, change-event-store, change-store-pg, chat-panel, chat-panel-spawn-render, chat-session-stream, chat-spawn-trace-events, chat-sse-mount, claim-gated-spawn, claim-store-work-time, claim-wisp-cold-start, cloud-sql-admin-rest, collapsed-suggestion-view, colour-by-subagent, compose-build-command, credential-broker, declare-presence, declared-edge-drift-report, desktop-build-route, desktop-launch-preconditions, dogfood-probe-mrfuze9m, drift-reads-store, event-sourced-store-seam, experience-rollout-guardrails, gate-emits-change, headless-session-runner, inline-comment-thread, leaf-tool-surface, library-dive-body, library-drawer-shell, library-finder, library-focus-subgraph, live-story-island-refresh, local-backend-boot, local-credential-wiring, map-build-seeds-terminal, member-suggest-write-policy, model-runtime-seam, multi-turn-transcript, node-resolve-report, noticeboard-cli, orchestrator-composition, orientation-tool-surface, owned-turn-loop, presence-store, pty-session-manager, r3f-world-spike, render-claim-as-wisp, repo-picker-panel, repo-selection, review-mode-toggle, review-refresh-feed, routed-node-real-dispatch, seed-corpus-scripts, shared-forest-connection, source-drift, spawn-deps-composition, spawn-tool-surface, story-author-spawn, suggestion-edit-store, take-claim-at-spawn, terminal-dock-panel, terminal-dock-seed, transcript-reset, tree-view, uat-bound-command-adoption, uat-machine-gate-resolution, uat-machine-proof-binding, verdict-glyphs, verdict-line, web-experience-sync, witnessable-verdict, worker-relocation, write-broker/,
  );
