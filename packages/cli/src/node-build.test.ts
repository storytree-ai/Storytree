...
test("node resolve on library-cli shows its arc explicit-ID REAL proof", async () => {
  // library-cli's focused real arm proves that an explicitly authored arc id is refused before
  // truncation can silently change it, while the broader mapped library still follows its Adopt gates.
