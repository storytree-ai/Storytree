...
test("node build without an id, and bare `node`, are help/guidance", async () => {
  ...
  // library-lens-minimise (BROWNFIELD studio vitest re-author of LibraryDrawer.tsx — the bottom
  // handle bar, minimise-to-handle/restore with body intact, the inc-8 bottom selection-preview
  // strip retired, the flag gate surviving both states). The two-pane TreeView mount + the
  // seed-packet look are the story's operator-attested UAT leg (shared inc-9+10 sitting).
