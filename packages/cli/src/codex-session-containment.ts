...
function Write-Atomic([string] $Target, [string] $Content) {
  ...
  try {
    ...
    # [NullString]::Value, NOT $null: Windows PowerShell 5.1 binds $null to a [string] parameter as
    # the EMPTY STRING, so Replace(..., $null) throws "The path is not of a legal form." before it
    # touches the disk. That made every RE-install impossible while the first install on an empty
    # managed directory passed, because only the existing-target branch reaches Replace at all.
...
export function buildCodexContainmentBundle(
  ...
  // a user-writable executable into the hook command — and under ADR-0364 the managed hook is the
  // ONLY fence, so anyone who can replace its interpreter can replace the fence. This is checked
  // only for a bundle someone means to INSTALL (`codexPayload` configured), because the dry run is
  // deliberately runnable from any shell for inspection, and refusing there would remove a surface
  // that costs nothing. Generate an installable bundle through the managed payload Node instead.
  if (codexPayload !== null && !insidePath(args.managedDir, args.managedNodePath)) {
    return {
      ok: false,
      reason:
        `an installable bundle must be generated by the administrator-owned Node under ${args.managedDir}, ` +
        `not ${args.managedNodePath} — the hook command is built from the RUNNING interpreter, and the ` +
        "managed hook is the only fence ADR-0364 leaves standing",
    };
  }
  ...
  const operatorReadme = [
    ...
    "",
    "HOW THIS BUNDLE IS ACTUALLY INSTALLED — the four traps, each of which silently produces a bundle",
    "that generates cleanly and then cannot be installed. Every one cost a real attempt.",
    "  1. Generate through the MANAGED Node, not whatever Node is on your PATH. The hook command and",
    "     the toolchain prefix are both built from the RUNNING interpreter's path, so generating from",
    "     an ordinary shell bakes a user-writable Node into a machine-wide boundary — and under",
    "     ADR-0364 the hook is the ONLY fence, so its interpreter must not be user-writable:",
    `       ${path.join(args.managedDir, "payloads", "node.exe")} packages/cli/launch.mjs \\`,
    "         write-authority codex --pg \\",
    "         --codex-payload <staged codex.exe> --worktree-create-payload <staged node.exe> \\",
    "         --toolchain-payload <staged pnpm.cjs>",
    "  2. All THREE payload flags are required for an installable bundle. Omit --codex-payload and",
    "     `install` refuses at its first pin check; omit --worktree-create-payload and `bootstrap`",
    "     does. A bundle missing them still prints in full, which is what makes this easy to miss.",
    "  3. Pin the Codex executable that sits beside its own `codex-resources` directory, NOT a flat",
    "     copy of the same bytes. The binaries are identical; only the LAYOUT lets it find",
    ...
    "consequence of ADR-0364's standing grant, not a hang; drain stale worktrees to keep it bounded.",
    "",
...
export async function codexSessionContainmentCommand(
  opts: {
    ...
    /**
     * `--codex-payload <absolute path>` — the administrator-owned native Codex executable already
     * staged under the managed payloads directory. The actuator's `install` verb verifies this pin
     * before it writes anything, so a bundle generated without it can never be installed at all.
     */
    readonly codexPayload?: string;
    /**
     * `--worktree-create-payload <absolute path>` — the administrator-owned managed NODE executable
     * the generated worktree creator runs under. The creator SCRIPT is hashed by the bundle itself
     * and pinned as this payload's fixed argument, so this flag names `node.exe`, not the `.mjs`.
     */
    readonly worktreeCreatePayload?: string;
  },
  ...
  if (opts.help) {
    return {
      ...
      body: [
        ...
        "  --codex-payload <absolute path>       pin the staged native Codex executable. The",
        "      actuator's `install` verb refuses without it, so a bundle generated without this flag",
        "      is a dry run that can never be installed.",
        "  --worktree-create-payload <absolute path>  pin the staged managed node.exe the worktree",
        "      creator runs under — NOT the .mjs, which the bundle hashes and pins itself. The",
        "      actuator's `bootstrap` verb refuses without it.",
      ].join("\n"),
      next,
    };
  }
  ...
  // Each `--*-payload` flag mints its pin from the file itself, so an operator never transcribes a
  // digest by hand. Refuse loudly rather than degrading: someone who passed a flag is asking for a
  // bundle that CARRIES that payload, and silently emitting one without it would be reported as
  // success while the actuator still refuses at its first `Assert-PinnedPayload`.
  const pinStaged = (
    flag: string,
    target: string,
  ): { readonly path: string; readonly sha256: string } | { readonly ok: false; readonly body: string } => {
    const hash = io.sha256File;
    if (hash === undefined) return { ok: false, body: "REFUSED — this IO cannot hash a staged payload" };
    if (!path.isAbsolute(target)) return { ok: false, body: `REFUSED — ${flag} must be an absolute path` };
    try {
      ...
      return {
        ok: false,
        body:
          `REFUSED — could not hash ${flag} at ${target}: ${String(error)}. ` +
  ...
  if (opts.toolchainPayload !== undefined) {
    const pinned = pinStaged("--toolchain-payload", opts.toolchainPayload);
    if ("ok" in pinned) return { ok: false, body: pinned.body, next };
    toolchainPayload = pinned;
  }

  // Without these two the generated actuator parses and installs NOTHING: `install` refuses at
  // `Assert-PinnedPayload 'Codex payload'` and `bootstrap` at the worktree-create payload, so every
  // other artifact in the bundle is unreachable. They were the last operator-side gap — the repo
  // could describe the boundary but not emit an installable one.
  let codexPayload: { readonly path: string; readonly sha256: string } | undefined;
  if (opts.codexPayload !== undefined) {
    const pinned = pinStaged("--codex-payload", opts.codexPayload);
    if ("ok" in pinned) return { ok: false, body: pinned.body, next };
    codexPayload = pinned;
  }

  // The managed NODE executable, not the creator script: the bundle hashes the generated
  // `storytree-codex-worktree-create.mjs` itself and pins it as this payload's fixed argument, so an
  // operator who points this at the script gets a refusal rather than a subtly wrong command.
  let worktreeCreatePayload: { readonly path: string; readonly sha256: string } | undefined;
  if (opts.worktreeCreatePayload !== undefined) {
    const pinned = pinStaged("--worktree-create-payload", opts.worktreeCreatePayload);
    if ("ok" in pinned) return { ok: false, body: pinned.body, next };
    worktreeCreatePayload = pinned;
  }
  ...
  const bundle = buildCodexContainmentBundle({
    ...
    ...(codexPayload === undefined ? {} : { codexPayload }),
    ...(worktreeCreatePayload === undefined ? {} : { worktreeCreatePayload }),
  });
