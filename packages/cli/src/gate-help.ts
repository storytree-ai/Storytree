...
export function renderGateHelp(): string {
  ...
  return [
    ...
    "  pnpm gate:bg              the same run, DETACHED — returns at once with a handle; pipe it freely",
    "  storytree dispatch <handle> --wait   block on that handle, exit with THE GATE's own status",
    "",
    "flags:",
