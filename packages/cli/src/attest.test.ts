...
class FakeStore implements AttestationStoreLike {
  ...
  history_: StoredAttestation[] = [];
  async record(att: Attestation): Promise<Attestation> {
    this.recorded.push(att);
    return att;
  }
  async history(_testId: string): Promise<StoredAttestation[]> {
...
function deps(over: Partial<AttestDeps> = {}): AttestDeps {
  return {
    store: new FakeStore(),
    loadUatTestCriteria: (storyId) =>
      storyId === "s" || storyId === "uat-attestation"
        ? [
            {
              criterionId: C1,
              revisionId: R1,
...
test("record human relay: signer = operator, relayedBy = the scribing session", async () => {
  ...
  const env = await attestCommand(
    { mode: "record", storyId: "uat-attestation", testId: C1 },
...
test("record: defaults are outcome=pass, witness=human", async () => {
  const d = deps();
  await attestCommand({ mode: "record", storyId: "s", testId: C1 }, {}, d);
  const rec = (d.store as FakeStore).recorded[0]!;
...
test("fail-closed signer: an unresolved signer refuses, recording nothing", async () => {
  ...
  const env = await attestCommand(
    { mode: "record", storyId: "s", testId: C1 },
    { witness: "human" },
    d,
  );
...
test("invalid outcome / witness is refused", async () => {
  const d = deps();
  const bad1 = await attestCommand(
    { mode: "record", storyId: "s", testId: C1 },
    { outcome: "maybe" },
    d,
  );
  assert.equal(bad1.ok, false);
  const bad2 = await attestCommand(
    { mode: "record", storyId: "s", testId: C1 },
    { witness: "either" },
    d,
  );
...
test("missing criterion id is refused", async () => {
  const env = await attestCommand({ mode: "record", storyId: "s", testId: undefined }, {}, deps());
  assert.equal(env.ok, false);
  assert.ok(env.body.includes("needs a story id and criterion id"));
});

// ── offline ────────────────────────────────────────────────────────────────────

test("offline (store null): record and list both refuse, pointing at --pg", async () => {
  const rec = await attestCommand(
    { mode: "record", storyId: "s", testId: C1 },
    {},
    deps({ store: null }),
  );
