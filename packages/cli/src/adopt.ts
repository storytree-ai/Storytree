...
export function adoptHelp(): Envelope {
  return {
    ...
    body: [
      ...
      "the green — always the machine, never you and never a model). approvedBy = who decided to adopt, and",
      "it is carried by the reliability-GATE verdicts only: a machine UAT leg's check was already declared",
      "and already bound to that journey, so its verdict records NO approver at all (ADR-0408).",
      "It refuses ANY status but `mapped` (ADR-0417 D4: authored `proposed` is not evidence a story entered",
      "adoption — sign a machine UAT leg at `storytree uat run <story> --pg` and one gate at `adopt gate`),",
      "a story with no",
