...
/**
 * The SEED SCOPE (ADR-0263): the kinds `apps/studio/data/knowledge.json` is the canonical home of, and
 * therefore the only kinds any seed ceremony carries in either direction — `export-corpus` up,
 * `sync-corpus` down, and the `check:corpus-sync` / `check:corpus-content` gate reports over both.
 *
 * **An ALLOWLIST, deliberately — and that shape is the decision, not an implementation detail.** This
 * was a denylist ("every structured kind except agent/template/ephemeral"), which meant a kind
 * introduced later joined the seed export by DEFAULT, with nobody deciding it should. Three did:
 * `friction` (2026-07-06), `arc` (2026-07-11), and `uat-criterion` (2026-07-18) all post-date
 * `export-corpus.ts` (2026-06-27), and only `plan` — excluded in the same commit that added `arc` —
 * was ever ruled on. Measured on 2026-07-29 that left **222 transient artifacts** queued to be written
 * into a seed that has never held one (+6074/-108 lines, 584 KB → 2.17 MB).
 *
 * Two properties make the accumulation worse than a large diff, and they are why the exclusions below
 * are structural rather than a matter of taste:
 *  - **The seed cannot delete.** `computeExportedSeed` never removes an entry (ADR-0120's invariant),
 *    and `diffCorpusContent` skips any seed id live has no row for. An exported friction item that is
 *    later adjudicated away live leaves a seed entry that is permanent AND invisible. The kinds that
 *    DRAIN are exactly the ones a never-deleting mirror cannot track.
 *  - **They are edited by ordinary ceremony.** An `arc` gains an increment-log entry on every landing
 *    (ADR-0183 D1), a `friction` item gets `route`/`routeReason` at adjudication (ADR-0168 D5), a
 *    `uat-criterion` is stamped as legs are attested. Each such edit on a SEEDED artifact is +1 on
 *    `check:corpus-content`'s value-drift axis, so seeding them would turn a slow editorial backlog
 *    into a counter driven by routine work — undrainable by construction (ADR-0252 D3).
 *
 * OUT, each for a decision on the record: `agent` (seed-canonical, owned by `sync-agents`, ADR-0055) ·
 * `template` (generated by `libraryTemplates()`, ADR-0210) · `plan` (ephemeral, ADR-0183 D2) ·
 * `friction` (transient signal; the durable essence graduates OUT, ADR-0168/0095) · `arc` (initiative
 * state and pointers, "not durable guidance", ADR-0183 D1) · `uat-criterion` (work-hierarchy tier,
 * which ADR-0120 D4 already kept out of the knowledge tier).
 *
 * A NEW KIND IS OUT UNTIL DELIBERATELY ADMITTED — add it here, with its reason, or it stays live-only.
 */
export const SEED_SCOPE_KINDS: ReadonlySet<string> = new Set<KnowledgeKind>([
  "definition",
  "principle",
  "pattern",
  "guardrail",
