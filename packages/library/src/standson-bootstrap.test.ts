...
import { findDependsOnCycles, dependsOnNodes } from "./knowledge-dag.js";
import {
  KNOWLEDGE_TIERS,
  projectDependsOnFromCitations,
  type CitationSource,
...
function cyclesInPlan(plan: ReturnType<typeof projectDependsOnFromCitations>): string[][] {
  return findDependsOnCycles(
    dependsOnNodes(plan.edges.map((e) => ({ id: e.id, doc: { dependsOn: [...e.dependsOn] } }))),
  );
}
...
test("library-standson-bootstrap-seeds-only-down-tier-citations: strictly-more-foundational targets seed an edge, nothing else does", () => {
  ...
  assert.deepEqual(plan.edges, [
    { id: "an-agent", dependsOn: ["asset:a-principle", "doc:decisions/0223-the-knowledge-dag.md"] },
  ]);
...
test("library-standson-bootstrap-seeds-only-down-tier-citations: a target cited twice seeds one edge", () => {
  ...
  assert.deepEqual(plan.edges, [{ id: "an-agent", dependsOn: ["asset:a-principle"] }]);
});
...
test("library-standson-bootstrap-is-acyclic-by-construction: a deliberately cyclic citation web projects clean", () => {
  ...
  assert.ok(findDependsOnCycles(web).length > 0, "fixture must contain citation cycles");
...
test("library-standson-bootstrap-is-acyclic-by-construction: every seeded edge strictly descends the tier order", () => {
  ...
  for (const edge of plan.edges) {
    ...
    for (const pointer of edge.dependsOn) {
...
test("ADR-0365 D1: uat-criterion sits at tier 6 — it seeds DOWN-tier and never at its tier-6 peer `increment`", () => {
  ...
  assert.deepEqual(plan.edges[0]?.dependsOn, ["asset:a-principle"]);
  assert.equal(
...
test("ADR-0365 D2: an increment's arc containment is NOT a dependsOn edge — only its down-tier citations seed", () => {
...
test("library-standson-bootstrap-never-overwrites-authored-edges: an authored edge survives an EXTENDING pass, first and in order", () => {
  ...
  const plan = projectDependsOnFromCitations([
    row("curated-agent", "agent", ["asset:a-principle"], { dependsOn: ["asset:a-guardrail"] }),
    // An EMPTY array is not authorship — it records no edge, so this one is seeded from empty.
    row("empty-agent", "agent", ["asset:a-principle"], { dependsOn: [] }),
    row("a-principle", "principle", []),
  ...
  assert.deepEqual(plan.edges, [
    ...
    { id: "curated-agent", dependsOn: ["asset:a-guardrail", "asset:a-principle"] },
    { id: "empty-agent", dependsOn: ["asset:a-principle"] },
  ]);
...
test("library-standson-bootstrap-never-overwrites-authored-edges: an artifact with nothing to add is a NO-OP, not an extension", () => {
  const plan = projectDependsOnFromCitations([
    ...
    row("settled-agent", "agent", ["asset:a-principle"], { dependsOn: ["asset:a-principle"] }),
    row("a-principle", "principle", []),
  ]);
...
test("ADR-0373: a uat-criterion's refs seed — the field ADR-0365 measured at tier 6 seeding zero", () => {
  ...
  assert.deepEqual(plan.edges, [
    { id: "a-criterion", dependsOn: ["asset:a-principle", "asset:a-process"] },
  ]);
});
