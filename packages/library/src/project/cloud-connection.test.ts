 ...
 * again, up to five times, and each reset is reported. The stand-in is what resets, not a Cloud
 * SQL instance, so this decides nothing about how storytree should treat a reset from a real one.
 */
async function loginRefusalOf(
  ...
  for (let tries = 1; ; tries++) {
    try {
      ...
      if (codeOf(error) === "ECONNRESET" && tries < 5) {
