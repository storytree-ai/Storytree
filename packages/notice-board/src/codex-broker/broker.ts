 ...
 *
 * This module is PURE — no `node:net`, no `pg`, no `node:child_process`. The resident server and the
 * real ledger live in `resident.ts` (composed by `codex-claim-broker-entry.ts` and, since ADR-0375,
 * the storytree desktop backend); the client the bootstrap dials lives in `client.ts`. Keeping the
 * grammar and the decisions here is what makes the refusals provable without a socket or a database.
 *
 * Moved here (not copied) from `packages/cli/src/codex-claim-broker.ts` by ADR-0375, because
 * `apps/desktop` may not import `@storytree/cli` (ADR-0112) but already depends on
 * `@storytree/notice-board`, which already owns the claim ledger this module drives.
 * `packages/cli/src/codex-claim-broker.ts` is now a thin re-export shim.
 */

import { claimGrade, type ClaimDocT, type ClaimResult } from "../claim.js";
...
/**
 * The live claims held by ONE named session — the read the managed hook's fence runs on every covered
 * tool call (ADR-0375).
 *
 * ## Why this is not the `claimsFor` ADR-0368 D4 refused to broker
 *
 * D4 left the ceremony's `claimsFor(unitId)` unbrokered because its consumer renders a board digest to
 * an OPERATOR, where an empty answer reads as "no other sessions" — a claim the broker cannot make, so
 * the client throws instead. This verb is the opposite case in every respect that mattered there:
 *
 * - It is **session-scoped, not unit-scoped**: it answers "what does THIS session hold", never "who
 *   else is on this unit", so it cannot understate other sessions because it never speaks about them.
 * - Its consumer is the **hook**, which reads an empty list as DENY. Under `claimsFor` an empty answer
 *   overstated safety to a human; here it withholds authority from a machine. The two failure
 *   directions are inverses.
 *
 * What must still never happen is an ERROR degrading to an empty list: that would be an outage wearing
 * the costume of a real answer. It is refused structurally — a fault answers `ok: false` and the
 * client THROWS (`claimsForSession`), so "the broker could not tell me" can never be spelled `[]`.
 */
export interface BrokerClaimsRequest {
  readonly verb: "claims";
  readonly sessionId: string;
}

export type BrokerRequest =
  | BrokerTakeRequest
  | BrokerPromoteRequest
  | BrokerReleaseRequest
  | BrokerClaimsRequest;
...
export type BrokerResponse =
  ...
  | { readonly ok: true; readonly verb: "claims"; readonly claims: readonly ClaimDocT[] }
  | {
      readonly ok: false;
...
export interface BrokerLedger {
  ...
  /**
   * The LIVE claims one session holds — `PgClaimStore.claimsBySession`, which already filters to live
   * rows. The hook's fence runs on this and on nothing else, so a stale read here is a fence that
   * admits a write whose authority has already been released.
   */
...
export function parseBrokerRequest(raw: unknown): BrokerRequest {
  ...
  if (verb === "claims") {
    assertExactKeys(source, ["protocolVersion", "verb", "sessionId"]);
    const sessionId = readString(source, "sessionId");
    if (!SESSION_ID.test(sessionId)) throw new Error("sessionId is not a well-formed minted session id");
    return { verb: "claims", sessionId };
  }

  throw new Error('verb must be one of "take", "promote", "release" or "claims"');
}
...
export async function handleBrokerRequest(
  ...
  try {
    ...
    if (request.verb === "claims") {
      // No narrowing, and deliberately none to add: this verb reads, it never grants. The caller
      // names a session and learns what that session holds — which the notice board renders to every
      // member anyway. The authority decision is the HOOK's, taken against the identity GIT derives
      // for the process it is fencing, never against the sessionId a caller asked about. A request
      // naming someone else's session therefore buys nothing: the hook compares what comes back to
      // its own observed identity and refuses on a mismatch (`authority()`), so a forged question
      // yields an answer that cannot match.
      return { ok: true, verb: "claims", claims: await deps.ledger.claimsBySession(request.sessionId) };
    }
