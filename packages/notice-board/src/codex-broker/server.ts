/**
 * The resident broker's HTTP half — the loopback listener that joins the door (`door.ts`) to the
 * grammar and decisions (`broker.ts`).
 *
 * Deliberately free of `pg` and of any credential: the identity the broker holds is composed in
 * `resident.ts`, so this module can be started over a REAL socket against a fake ledger and its
 * transport refusals proven for what they are.
 */
