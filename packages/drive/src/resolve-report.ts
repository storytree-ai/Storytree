...
export interface ResolveRealReport {
  ...
  proofDisplay: string;
  /**
   * `custom-proof-command-red-accounting`: the classified basis of that proof route — see
   * `ProofRouteBasis`. `observes-another-file` is the one `--real` refuses before spending a turn.
   */
...
export function resolveReport(spec: NodeSpec): ResolveReport {
  ...
  const real: ResolveRealReport = {
    ...
    proofDisplay: resolvedProof.display,
    proofRouteBasis: resolvedProof.route.basis,
  };
