...
- An agent that dispatches asynchronous work — a background subagent, a background shell task, a sub-spawned helper — never ends its turn awaiting that work's completion notification. **The reason is not that the signal is unreliable.** It is addressed to the agent that dispatched it and reaches that agent exclusively; what makes it miss is the agent's own turn ending. Delivery rides the dispatcher's NEXT TOOL ROUND-TRIP and is flushed at a turn boundary rather than pushed into an idle agent, so an agent that stops taking tool calls stops being addressable, and its signal then surfaces at the nearest still-live ancestor — which becomes an involuntary router for a conversation it is not part of. Ending the turn is therefore not a way of waiting; it is the act that forfeits the result. So: while the work is live and you can still bound the wait, KEEP TAKING TURNS and the signal will reach you. When the work will outlast what you can honestly bound, do not stall and do not guess — complete on your own judgment THIS turn and HAND BACK A DISPATCH HANDLE: what was dispatched, WHERE its verdict will appear (a sentinel path or exit-status file agreed in advance, not a promise to remember), and the explicit statement that nobody has read it yet. A handle is not a verdict: until someone reads it the check is UNVERIFIED, never a pass (`asset:unrun-check-is-unverified-not-refuted`). The turn is the only unit of agency the agent controls, and spawning asynchronous work does not extend it.  — `storytree library artifact an-awaited-notification-is-not-a-turn-ending-state`
