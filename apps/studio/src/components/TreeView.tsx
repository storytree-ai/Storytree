...
/** Experiment 9's independent, exact, default-off comparison gate. */
export function readOrganicBranchBloom(search: string = defaultSearch()): boolean {
  return (
    new URLSearchParams(search).get('organicGrowth') ===
    'organic-branch-bloom'
  );
}
...
export function TreeView({
  ...
  const organicBranchBloom = useMemo(() => readOrganicBranchBloom(search), [search]);
  const scene = useMemo(
    () =>
      world
        ? buildScene(
  ...
  if (semanticGrowthDemo || organicPoseToPose || organicBranchBloom) {
    return (
      <SemanticGrowthDemo
