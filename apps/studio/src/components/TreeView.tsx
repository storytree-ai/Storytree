...
/** Experiment 4: exact, collision-free comparison gate. Near misses and every clean route stay on
 * the ordinary forest; the value names the industry technique rather than an adoption decision. */
export function readOrganicKeyPoseBlend(search: string = defaultSearch()): boolean {
  return (
    new URLSearchParams(search).get('organicGrowth') === 'organic-keypose-blend'
  );
}
...
export function TreeView({
  ...
  const organicKeyPoseBlend = useMemo(() => readOrganicKeyPoseBlend(search), [search]);
  const scene = useMemo(
    () =>
      world
        ? buildScene(
  ...
  if (semanticGrowthDemo || organicPoseToPose || organicKeyPoseBlend) {
    return (
      <SemanticGrowthDemo
