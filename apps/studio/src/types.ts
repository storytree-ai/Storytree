...
//
// What the traversal replay panel's occupancy bar plots.
//
...
//
// The per-machine WINDOW LIST shapes that used to sit here — `ContextWindowEntry`,
// `ContextHelperEntry`, `ContextScanPayload`, `ContextWindowsPayload` — retired with the standalone
// Context tab (ADR-0456 D1). The fold that produced them survives in
// `@storytree/context-traversal-transcript` and `storytree context` reads its sibling; what went is
// the wire, because nothing in the browser asks that question any more.
