...
describe('the marks are ADR-0411 D3’s', () => {
  it('context-window-meter-draws-the-two-marks-as-colour: places the soft mark at 400k and the hard mark at 500k', () => {
    expect(SOFT_MARK_TOKENS).toBe(400_000);
    expect(HARD_MARK_TOKENS).toBe(500_000);
  });

  it('context-window-meter-draws-the-two-marks-as-colour: bands a reading at exactly a mark as being IN that band, and just below it as out', () => {
  ...
  it('context-window-meter-draws-the-two-marks-as-colour: gives each band the decision ADR-0411 D3 attaches to it, not a paraphrase of the number', () => {
...
describe('the track ceiling', () => {
  it('context-window-meter-scales-every-window-on-one-track: leaves headroom above the hard mark, so at-the-limit and past-it cannot draw identically', () => {
  ...
  it('context-window-meter-scales-every-window-on-one-track: is ONE ceiling for every reading, chosen from the fullest, so two meters compare by eye', () => {
  ...
  it('context-window-meter-scales-every-window-on-one-track: holds the base ceiling until a reading exceeds it, then steps up to contain it', () => {
...
describe('one reading splits into three coloured portions', () => {
  it('context-window-meter-draws-the-two-marks-as-colour: splits at EXACTLY a mark — only the excess above one is ever coloured for it', () => {
  ...
  it('context-window-meter-scales-every-window-on-one-track: never draws past the track, even when handed a stale ceiling', () => {
    const s = meterSegments(2_000_000, BASE_SCALE_TOKENS);
    expect(s.calmFraction + s.softFraction + s.hardFraction).toBeCloseTo(1, 10);
  });

  it('context-window-meter-draws-the-two-marks-as-colour: draws nothing for an empty window and never a negative segment', () => {
  ...
  it('context-window-meter-scales-every-window-on-one-track: falls back to the base ceiling rather than dividing by a zero scale', () => {
    expect(meterSegments(300_000, 0).calmFraction).toBeCloseTo(300_000 / BASE_SCALE_TOKENS, 10);
  });
});

describe('readouts', () => {
  it('context-window-meter-reports-its-own-limits: formats tokens the way the replay panel does — one house format, not a second copy', () => {
    expect(formatTokens(500_250)).toBe('500.3k');
    expect(formatTokens(1_240_000)).toBe('1.24M');
  });

  it('context-window-meter-reports-its-own-limits: labels an age at the coarsest unit still true, and refuses to invent one for an undated read', () => {
