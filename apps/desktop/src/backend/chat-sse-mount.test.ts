...
//
// It `implements` the seam rather than being asserted into it (anti-slop
// `no-chained-type-assertions`, inc-09). The seam is reached through `loadFixtureCorpus`'s own
// parameter, so the compiler holds this double to the REAL `Store` contract without desktop having
// to declare a dependency it does not otherwise need. That is the difference between a partial fake
// and a double: a method the mount starts calling is now a compile error here, where under the
// assertion chain it was a runtime `undefined is not a function` inside an SSE stream.
...
class FixtureStore implements CorpusStoreSeam {
  ...
  /**
   * The ADR-0352 field-scoped write, implemented rather than stubbed: `patchDoc` is part of the
   * `Store` contract, and a double that answered `null` to every patch would let a mount that
   * started patching pass this suite while doing nothing.
   */
  async patchDoc(input: {
    ...
    // `actor` is optional on the seam and must be ABSENT, never present-and-undefined — one
    // unconditional spread over a base, chosen by a ternary, satisfies both anti-slop rules.
...
test(
  ...
  async () => {
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: queryYielding([OK_SDK_RESULT]) });
...
test(
  ...
  async () => {
    ...
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: capturingQuery });
...
test(
  ...
  async () => {
    ...
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: sentinelQuery });
...
test(
  ...
  async () => {
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: queryThrowing("scripted SDK failure") });
...
test(
  ...
  async () => {
    ...
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: blockingQueryFn });
...
test(
  ...
  async () => {
    ...
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: capturingQuery, runner });
...
// LANDING SEAM — RETIRED (ADR-0175). ADR-0152's merge-ceremony surface (`run_gate` /
// `open_landing_pr` / `poll_pr_checks`) was forwarded through this mount until ADR-0174 retired the
// in-app interactive orchestrator and ADR-0175 ruled that "the spawn and landing surfaces (which drove
// story work) do not belong to a help agent and retire with the interactive orchestrator" — the mount
// takes no `landing` dep any more, and the modules behind it are deleted.
//
// The positive `csm-forwards-landing-deps` test went with the surface, exactly as the ADR-0155 /
// PR #587 propose/accept tests did. What survives is the NEGATIVE assertion below, which exists to
// keep it gone: no session this mount starts may advertise a `mcp__landing__*` tool. The wider guard
// (the modules are deleted; nothing in the chain re-composes them) is landing-surface-retired.test.ts.
//
// Why it matters beyond tidiness: `open_landing_pr` cut a FRESH branch on an already-merged branch and
// kept the session working (ADR-0163 D3 Gap B1's remedy over ADR-0142's post-merge leg). ADR-0271
// ended that shape — a session's working life ends where its PR merges — and this sidecar renders the
// SAME session-orchestrator agent, so a wired tool contradicted its own prompt.
test(
  "csm-no-landing-surface: no landing tool is advertised — the merge-ceremony surface is retired (ADR-0175)",
  async () => {
    let capturedOptions: unknown;
    const capturingQuery: QueryFn = ({ options }) => {
      capturedOptions = options;
    ...
    assert.ok(
      ...
      `the landing surface is retired (ADR-0175) — no mcp__landing__* tool may be advertised; ` +
        `got: ${JSON.stringify(allowed)}`,
    );
  },
);
...
test(
  ...
  async () => {
    ...
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: capturingQuery, inspect: inspectDepsDouble() });
...
test(
  ...
  async () => {
    ...
    const handler = createChatSseMount({
      store: FIXTURE_STORE, queryFn: capturingQuery, maxTurns: 25 });
