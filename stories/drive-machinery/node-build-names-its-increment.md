...
**The complete breakage set, measured across both suites at two authored implementations.** Both
suites were run in full at attempt 1's authored commit `a4e3dc5` and at attempt 3's `c63e7f1`.
- `@storytree/cli` passed 4212/4212 at both. Both attempts had made the CLI test updates listed
  below under "The existing tests in scope", and those updates are still required: the tests pin
  commands this unit changes.
- `@storytree/drive` failed exactly one parent test at each:
  `node-build-takes-a-revision-and-names-the-record-it-leaves` in
  `packages/drive/src/node-build-revise-test.test.ts`. Each attempt left a DIFFERENT one of its subtests
  un-updated.

So in `node-build-revise-test.test.ts` exactly TWO subtests need updating, and no other existing drive
test does:
1. `a genuinely written, matching revision is never refused by the revision read — the build proceeds to the DB preflight`:
   its `nodeBuild` call passes `increment` and `innerLoopReads`. Attempt 1 missed this.
2. `--revise-test without --real is refused before the signer resolves or the spec loads, naming --revise-test and --real`
   (around line 227): its expected `next` command gains `--increment <increment-id>`, exactly as the
   implementation prints it:
   `storytree node build no-such-node-fixture-for-revise-test --real --increment <increment-id> --revise-test some-prior-run-id`.
   Attempt 3 missed this.

Both keep their titles byte-for-byte; only their bodies change.
...
Attempt 3 (run real-mu4cxq37) failed closed at GATE: the drive regression suite was red on the second existing revise-test subtest the test author left un-updated; the per-test review passed at red and green.
