...
16. **`codex-envelope-reports-feedback-runs`** — the build envelope reports a Codex leaf's feedback runs from the spine's own record
    ...
    - **proven by —** `packages/drive/src/node-build-codex-feedback.test.ts` through contract [`node-build-renders-codex-feedback-runs`](node-build-renders-codex-feedback-runs.md), not yet signed.
