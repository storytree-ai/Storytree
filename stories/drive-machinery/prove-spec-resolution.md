...
12. **`test-revision-reaches-only-the-author-test-brief`** — a test revision handed to a REAL build reaches the AUTHOR_TEST brief, after everything that brief says today, and no other brief
    ...
    - **proven by —** contract [`real-brief-carries-test-revision`](real-brief-carries-test-revision.md): a new `packages/orchestrator/src/resolve-prove-spec.revision.test.ts` (proposed; not yet signed)
