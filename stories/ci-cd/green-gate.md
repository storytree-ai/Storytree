...
outcome: "A PR's verify job proves it against the merge of branch+main — manifest, CLAUDE.md/agents sync, typecheck, test, build — and a red anything blocks the merge."
status: proposed
...
3. **`generated-views-in-sync`** — the three surface/sync gates catch drift
   - **asserts —** `check:manifest` fails on an unlisted root/loose-doc surface; `check:claude` fails
     ...
     (ADR-0051); `check:agents` fails when a `.claude/agents/*.md` drifts from its library agent
     (ADR-0052) — each is a real `verify` step, not advisory.
