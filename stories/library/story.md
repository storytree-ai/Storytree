...
5. **Whether this story should later be driven through the REAL prove-it-gate to earn `healthy` (partly addressed — affordance landed, live red→green still pending).** Today nothing here is `healthy` (the gate never drove these proofs). The `--real` **affordance now exists** ([ADR-0092](../../docs/decisions/0092-gate-as-proof-for-a-machine-witnessed-story-s-own-uat-node.md)): the gate-as-proof `real:` arm on this machine-witnessed story's own UAT node plus brownfield `real:` arms on the 7 capabilities make `story build library --real` buildable, so the studio offers it (ADR-0090/0091). But the affordance is not the proof: the library is a mature brownfield artifact with no genuine live red — its completeness spec reads GREEN already and its capability tests predate the gate — so a live `--real` build observes already-complete at CONFIRM_RED and fails closed (ADR-0092's headline caveat). A real live red→green that upgrades the mapped capabilities to `healthy` therefore still requires either a story authored from an incomplete/scaffold state or a human-witness action by the owner; it has NOT happened yet. Confirm that earning `healthy` this way is the intended trajectory.
