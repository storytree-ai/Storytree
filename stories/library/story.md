...
# brownfield (`mapped`), so its honest path to green is **Adopt** — the `## Reliability Gates` below
# observe-and-signed (ADR-0085) — NOT a fail-closed `--real` Build over a mature artifact with no
# genuine live red. The studio's go-green affordance is now status-aware (ADR-0094): `mapped → Adopt`.
proof:
  command:
    file: pnpm
...
**Adopt ENTERS a proving process; it does not flip the crown green ([ADR-0097](../../docs/decisions/0097-brownfield-go-green-is-a-proving-process-adopt-enters-brown.md)).** Adopting these three `observe` gates flips the tier `mapped → proposed` ("adoption underway") and, via their `(covers:)` declarations, greens **six of the seven** capabilities. `healthy` stays non-authorable ([ADR-0020](../../docs/decisions/0020-red-green-enforcement-on-the-owned-loop.md)) — the authored `status:` stays `mapped`; the world's crown DERIVES green from the signed verdicts ([ADR-0040](../../docs/decisions/0040-verdict-derived-green-and-the-human-witness-signpost.md)) and only when **every** capability is `healthy` AND every own-proof obligation is signed ([ADR-0082](../../docs/decisions/0082-per-test-uat-tests-earn-green-by-declared-witness-story-uat.md) / [ADR-0083](../../docs/decisions/0083-author-defined-story-green-declared-obligations-machine-per.md) Fork A + ADR-0085). The **seventh capability, [`seed-corpus-scripts`](seed-corpus-scripts.md) (`proposed`), is covered by NO honest observe gate** — the library suite only smoke-imports it, and adopting a suite that does not exercise the code would be the rubber-stamp ADR-0085 bans. It needs the real work: a `_(gate: build-tests)_` gate driven genuinely red→green (the Layer-3 follow-on ADR-0097 names). Until then it holds the crown at `proposed`, which is exactly what makes a green crown MEAN the untested pocket got real coverage. No single gate greens the story; the `## Story UAT (would-be)` legs above are aspirational and do not block the crown (ADR-0097 §6).
