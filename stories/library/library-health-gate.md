...
Real collaborators, no stubs: the integration-flavoured proof is `packages/cli/src/health.test.ts:191-203` (passing): `loadCorpus` (the real `@storytree/store` seeder) into a real `InMemoryStore`, `queryDocs`, then `libraryHealth` — asserts `gateFailures()` is EMPTY (schema-conformance + retired-field + version-floor all clean on the stamped seed). That is exactly what makes `pnpm -r test` (ADR-0022) enforce migration/seed health offline, wiring two real collaborators (the health checks + the store's `loadCorpus`) with no stub.
