...
> implementations — `ClaudeAgentAuthor` (`sdk-author.test.ts`), `CodexPhaseAuthor`
> (`codex-author.test.ts`), and `OwnedLoopAuthor` in drive-machinery — and by the gate that consumes
> it as a TYPE only
