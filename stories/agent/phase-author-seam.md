...
> **Proof status (honest) — `proposed`.** At HEAD `phase-author.ts` is still a pure type module (no
> runtime, no test of its own to count): it declares `AuthoringPhase` (`"AUTHOR_TEST" | "IMPLEMENT"`),
> `LiveRuntime`, `AuthorResult` (`{ ok: true } | { ok: false; error; exhausted? }`), and the
> `PhaseAuthor` interface (`author(phase, prompt) → Promise<AuthorResult>`). Its behaviour is proven
> by its implementations — `ClaudeAgentAuthor` (`sdk-author.test.ts`), `CodexPhaseAuthor`
> (`codex-author.test.ts`), `PiPhaseAuthor` (`pi-author.ts`), and `OwnedLoopAuthor` in
> drive-machinery — and by the gate that consumes it as a TYPE only (`prove-it-gate.ts:18`). The
> proposed contract [`authoring-escalation-shape`](authoring-escalation-shape.md) gives the seam its
> first runtime value, a pure validator with a test of its own; once that contract lands the module is
> no longer pure types. No `healthy` — no signed verdict (ADR-0020).
...
## Escalation (proposed, ADR-0569)

A leaf that concludes the contract cannot be tested as specified, or that the test it was handed
cannot be satisfied as written, has no move today but to fail, and that failure reads exactly like bad
work. ADR-0569 D1 gives the seam a typed alternative, built by contract
...
- `parseAuthoringEscalation(phase, input)` is the only way a leaf builds one. It is pure, derives the
  ...
  IMPLEMENT escalation that does not quote the assertion it cannot satisfy. The package barrel
  publishes the validator and the type.
...
write a runtime with no knowledge of the gate's phase machine. The escalation validator, once built,
is proven directly by its contract's own test, `packages/agent/src/phase-author.test.ts`.
