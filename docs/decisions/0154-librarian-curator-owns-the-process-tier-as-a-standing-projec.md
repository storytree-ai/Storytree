...
2. **The CLI/pnpm surface is a declared projection of the process tier, gated on coverage — never on necessity.** Every `process` names its enacting entrypoint(s) in `surfaces` (a `storytree` command or a root `pnpm` script). A new advisory gate `check:surface-coverage` asserts (a) each surface a process names resolves to a real entrypoint, and (b) flags any operator-facing entrypoint with **no** process behind it (an orphan). It **WARNs, never blocks**, and runs local + CI — the established pattern of `check:coverage` / `check:agents-sync` / `check:corpus-sync`. The gate encodes the bijection; it never judges whether a command *should* exist.
   ...
   **CORRECTED IN PLACE 2026-07-27 — "WARNs, never blocks" no longer holds; nothing here was re-decided.** The gate now sets a non-zero exit code when its gap lists grow past a **two-axis drain ceiling** (`unresolved` and `orphan`, each redding on its own and never summed), baselined on the sweep of 2026-07-27 and tightening-only (**tightened 2026-07-28: BOTH axes now sit at ZERO** — `process:affected-pr-test-scope` was authored from ADR-0195, draining the one orphan the 2026-07-27 baseline had to carry, so there is no headroom left on either axis and a first orphan reds the gate; the ceiling numbers are `surface-coverage-drain.ts`'s, never this text's). This is not a reversal of this decision but the discharge of a *later* charter that reaches it: ADR-0252 D1's `warn-list-hygiene` instrument makes an unbounded WARN worklist the located defect itself, and D3 puts every instrument's backlog under its own drain ceiling — so a permanently-advisory `check:surface-coverage` became a thing that instrument locates, drained in ADR-0168 D4's shape. It was the second such worklist bounded, after `check:graduation-worklist`. **The load-bearing half of this decision stands unchanged and still binds the ceiling:** it gates BIJECTION DRIFT over what exists, and no count in it decides *which* commands should exist or which processes should be authored — that judgement remains D1's librarian-curator charter. Advisory-vs-blocking was a mechanism choice later re-chartered; *necessity is not gateable* was the decision, and it is untouched. Machinery + the measured evidence: `packages/cli/src/surface-coverage-drain.ts`.
...
- Cost: a standing obligation on the librarian pass (more to check before each merge) and a new gate to maintain. ~~Mitigated by keeping the gate advisory (WARN) so it never strands a PR, matching its sibling sync checks.~~ **Corrected 2026-07-27 (see the D2 correction):** the mitigation as written is void — the gate *can* now strand a PR. What replaced it is narrower and is the real mitigation: only **growth past the baselined ceiling** reds, so the standing backlog this ADR inherited never blocks, and a breach is discharged by a drain that is already in the operating discipline (author the missing `process`, fix the stale `surfaces` ref, or retire the entrypoint) — never by raising the number.
...
- **Not decided:** any change to who authors the work hierarchy (story-author) or the behavioural floor (guidance-curator); ~~making `check:surface-coverage` blocking (it starts advisory, like its siblings)~~ — **settled 2026-07-27, elsewhere and on a narrower question:** it is not blocking-per-finding (an individual gap still only WARNs, exactly as this ADR left it) but is now blocking-on-*growth* past a two-axis ceiling, under ADR-0252 D1/D3 rather than under this ADR; see the D2 correction; the exact `surfaces` field grammar for entrypoint references (a build-time detail for the gate unit).
