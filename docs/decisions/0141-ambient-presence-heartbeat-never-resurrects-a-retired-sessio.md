...
not `done`). The `reactivate: false` guard was OPERATIVE until the ADR-0200 arc's final
retirement increment DELETED the presence write path on 2026-07-17 (the presence store landed in
PR #765, the `events.session` tables dropped in #766, after the owner's inc-5 UAT attestation of the
claim-grade renders); the original body below is preserved unchanged as history.
