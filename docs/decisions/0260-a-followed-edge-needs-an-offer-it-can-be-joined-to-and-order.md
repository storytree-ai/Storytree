...
**Still not delivered:** the GUIDANCE change this decision reaches (agents must actually run the offered form, which is a behavioural ask rather than code) and the `doc:` blind spot, which stays declared rather than closed — and is now not merely declared but *drawn*, since every unfollowable offer renders as such in the picture instead of being silently absent from it.
