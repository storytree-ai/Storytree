...
"""Compose the twenty-four delivered panels into two comparison sheets a person can read at a glance.
    ...
    python combine.py [<source-dir>]      # default: ../../../.status-measure

Six STATES across, BEFORE above and AFTER below, one sheet per zoom. Reading ACROSS a row is the
whole colour vocabulary at once; reading DOWN a pair is what ADR-0462 changed.
...
figure derived from them was confounded. The inputs here are `status-measure.mjs`'s `getImageData`
...
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "..", "..", ".status-measure")
...
#: The six states, in the order the vocabulary reads them, with the token each wears AFTER.
STATES = [
    ("proposed", "proposed", "#d8c069 yellow"),
    ("building", "building", "#d8c069 — the SAME yellow"),
    ("mapped", "mapped", "#b3946a brown"),
    ("healthy", "healthy", "#8cb85e green"),
    ("unhealthy", "unhealthy", "#57544a black"),
    ("unknown", "unknown", "#9ca3af grey"),
]
ROWS = [("legacy", "BEFORE"), ("live", "AFTER")]
...
def sheet(zoom, out):
    ...
    for state, _, _ in STATES:
        for row, _ in ROWS:
            path = os.path.join(SRC, f"status-{row}-{state}-{zoom}px.png")
            if not os.path.exists(path):
                raise SystemExit(f"missing panel: {path} — run `pnpm --filter … measure-status` first")
...
for z in (2, 8):
    sheet(z, os.path.join(HERE, f"status-combined-{z}px.png"))
