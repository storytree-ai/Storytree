...
CONVERGENCE = os.path.join(REPO, "docs", "research",
                           "chapter2-compositor-convergence-2026-08-19")
sys.path.append(CONVERGENCE)
import compose_shared as SH  # noqa: E402  the ONE implementation of the nine below
...
# The nine functions below are FORWARDERS. Their bodies live once, in
# `chapter2-compositor-convergence-2026-08-19/compose_shared.py`, which takes `C` and this pass's
# paths as explicit parameters so that a shared implementation can never serve another pass's
# island. The public signatures are UNCHANGED on purpose: `delivery.py` and `dressed_fix.py`
# monkey-patch `paste_decor` on THIS module and rely on `compose_land` resolving it here, so the
# module attribute has to stay a patchable name with the shipped arity. That is also why
# `compose_land` itself is NOT shared — moving it would resolve `paste_decor` in the shared
# module's globals and silently disarm both patches, which is the PR #1393 failure exactly.

def rebind():
    return SH.rebind(C, ISLAND_PATH, LAND_PIECES)
...
def classify_decor(path, keys, meta):
    return SH.classify_decor(path, keys, meta)


def load_decor():
    return SH.load_decor(DECOR_PIECES)
...
def build_palette_dressed():
    return SH.build_palette_dressed(C, DECOR_META)
...
def paste_decor(canvas, alpha, piece, cx, cy, roles, role_map):
    return SH.paste_decor(C, canvas, alpha, piece, cx, cy, roles, role_map)
...
def assert_land_unchanged():
    return SH.assert_land_unchanged(C, compose_land, INTERIOR, ELEVATION_MODE)


# ---------------------------------------------------------------- the hero tree
def plant_tree(img, height):
    return SH.plant_tree(C, ISLAND, SWEEP, img, height)
...
def prepare(cells):
    return SH.prepare(C, ISLAND, ELEVATION_MODE, cells)


def require_one_state_per_generator(inputs):
    return SH.require_one_state_per_generator(inputs)
