...
> ⚠ **THESE NUMBERS WERE RESTATED ON 2026-08-18 AND THE TABLE NOW MATCHES THIS PASS'S OWN
> `order-and-caps-report.json`.** The report was already regenerated by PR #1393 — which replaced the
> plant positioner every row here was measured on — but this table was not, so for a day it quoted a
> pre-dispersion measurement while its own committed report said otherwise. The rows below are the
> report's. What the earlier table said, and why it moved, is attributed in
...
|---|---:|---:|---:|---:|---:|---:|
| **fixture** (`fork-spike-island`) | 115 | 41 (**35.7%**) | 8 (**7.0%**) | 22 → **0** | 338 → **534** | 34 968 both ways |
| **fixture driven all-`healthy`** | 148 | 53 (**35.8%**) | 8 (**5.4%**) | 28 → **0** | 452 → **709** | 34 968 both ways |
| **real corpus** (`context-traversal-capture`) | 181 | 65 (**35.9%**) | 12 (**6.6%**) | 32 → **0** | 624 → **959** | 30 477 both ways |
...
1. ~~**The real-corpus island's residual loss is 17.2%, not 7.1%.**~~ ✅ **CLOSED 2026-08-18, and the
   answer is that the gap was a SECOND DEFECT rather than a property of the island.** The 17.2% was
   measured against the placements the affine-CRC32 scatterer produced; PR #1393 replaced that
   positioner the next morning, and on the same island, the same compositor and the same seed the
   real-corpus rate is **6.6%** against the fixture's **7.0%** — so the real surface is not "more than
   twice the fixture", it is at or below it. The attribution, its signed prediction and the 2x2 that
   separates the two defects are in `chapter2-delivery-residual-2026-08-18/`. ⚠ The reading this gap
   offered — that the real island "packs more decor onto fewer, smaller cells" — was **half wrong**:
   it does carry more plants per cell (1.12 against 0.54, which turns out to be the whole
   explanation), but the cells are NOT smaller. Mean cell area is 150.5 ground units² on the fixture
   against 152.0 on the real island, 1% apart; the real island simply has fewer of them.
