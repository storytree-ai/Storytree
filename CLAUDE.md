...
**Workflow.** **session_start:** read CLAUDE.md and the notice board; declare presence (`storytree noticeboard declare --node <unit> --pg` — claim the CAPABILITY you are writing, the story only for cross-capability work, ADR-0270 D1); search the corpus just-in-time, never preload it.
