...
re-runs the copy, and no `pnpm gate` or CI rung checks the published object. The retired diagnostic
still exists at `packages/cli/src/check-dist-drift.ts` for an explicit operator check, but publishing
and verifying the result remain part of the manual ceremony. Fully automating the publish (D5's
"published by Cloud Build on release") is still the open follow-on below.
